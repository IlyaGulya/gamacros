# Examples for padjutsu-bit-mask

This directory contains examples demonstrating various use cases for the padjutsu-bit-mask library.
...
```bash
cargo run --example <example_name> --manifest-path crates/padjutsu-bit/mask/Cargo.toml
```

For example:
```bash
cargo run --example basic_usage --manifest-path crates/padjutsu-bit/mask/Cargo.toml
```
...
The examples use:
- `padjutsu_bit_mask`: The main library
- `padjutsu_bit_derive`: For the derive macro examples (where applicable)
