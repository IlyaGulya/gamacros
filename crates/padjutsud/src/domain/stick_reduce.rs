...
fn apply_stick_transition_intents(
    step: &mut DomainStep,
    controller_id: padjutsu_gamepad::ControllerId,
...
fn resolve_controller_mode(
    padjutsu: &Padjutsu,
    id: padjutsu_gamepad::ControllerId,
) -> ControllerMode {
    let has_buttons = padjutsu.controller_has_pressed_buttons(id);
    let left_stick =
        resolve_stick_state(padjutsu, id, padjutsu_workspace::StickSide::Left);
    let right_stick =
        resolve_stick_state(padjutsu, id, padjutsu_workspace::StickSide::Right);
    ...
    let has_repeats = padjutsu.controller_has_repeats(id) || has_stick_repeats;
...
pub fn resolve_controller_state(
    ...
    let right_stick =
        resolve_stick_state(padjutsu, id, padjutsu_workspace::StickSide::Right);
    let mode = resolve_controller_mode(padjutsu, id);
...
mod tests {
    ...
    use padjutsu_gamepad::{Axis, Button, ControllerInfo};
    use padjutsu_workspace::{AppRules, Profile};
    ...
    fn controller_state_is_idle_without_input() {
        ...
        let state = resolve_controller_state(&padjutsu, 1);
    ...
    fn controller_state_becomes_axis_active_on_stick_motion() {
        let mut padjutsu = Padjutsu::new();
        padjutsu.add_controller(controller_info(1));
        padjutsu.on_axis_motion(1, Axis::LeftX, 0.7);
    ...
    fn controller_state_becomes_buttons_active_on_button_press() {
        let mut padjutsu = Padjutsu::new();
        padjutsu.set_workspace(profile_with_common_rules());
        padjutsu.add_controller(controller_info(1));
        let _ = padjutsu.on_button_effects(1, Button::A, ButtonPhase::Pressed);
    ...
    fn push_controller_state_update_records_stick_transition() {
        ...
        let idle_state = resolve_controller_state(&padjutsu, 1);
        runtime_state.set_controller_state(1, idle_state);

        padjutsu.on_axis_motion(1, Axis::LeftX, 0.7);
        let next_state = resolve_controller_state(&padjutsu, 1);
