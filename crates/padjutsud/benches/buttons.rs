...
use padjutsu_gamepad::{Button, ControllerId, ControllerInfo};
use padjutsu_workspace::{AppRules, ButtonAction, ButtonRule, Profile, StickRules};
use padjutsud::app::{Effect, Padjutsu};
use std::sync::Arc;

fn build_profile_simple(button: Button, combo: KeyCombo) -> Profile {
    let mut rules = padjutsu_workspace::RuleMap::default();
    let mut app = AppRules::default();
    let mut buttons = padjutsu_workspace::ButtonRules::default();
    let mut chord = padjutsu_bit_mask::Bitmask::empty();
    chord.insert(button);
    buttons.insert(
        chord,
        ButtonRule {
...
pub fn bench_button_path(c: &mut Criterion) {
    let mut g = Padjutsu::new();
    let profile = build_profile_simple(
        Button::A,
        KeyCombo::from_key(padjutsu_control::Key::F1),
    );
    ...
    c.bench_function("buttons_press_release_single", |b| {
        b.iter(|| {
            ...
            g.on_button_with(
                id,
                button,
                padjutsud::app::ButtonPhase::Pressed,
                |a| {
                    match a {
                        Effect::KeyPress(_)
                        | Effect::Rumble { .. }
            ...
            g.on_button_with(
                id,
                button,
                padjutsud::app::ButtonPhase::Released,
                |a| {
                    match a {
                        Effect::KeyPress(_)
                        | Effect::Rumble { .. }
